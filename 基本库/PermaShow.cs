...
namespace LeagueSharp.Common
{
    ...
    public static class PermaShow
    {
        ...
        private static void CreateMenu()
        {
			...
			//修改的内容：添加设置文字大小
			var fontSize = new MenuItem("字体大小", "字体大小").SetValue(new Slider(16, 12, 20));

			placetosave.AddItem(moveable);
            placetosave.AddItem(bigwidth);
            placetosave.AddItem(smallwidth);
			placetosave.AddItem(fontSize);
        ...
        private static void PrepareDrawing()
        {
            ...
            try
            {
				...
				//修改的内容：修改字体大小为菜单选择，不再与菜单字体大小同步
				FontHeight = placetosave.Item("字体大小").GetValue<Slider>().Value;
				Quality = (FontQuality)
                    Enum.Parse(
                        typeof(FontQuality),
